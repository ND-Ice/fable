...
function Carousel({ images }: Props) {
  ...
  return (
    <div className="relative h-full w-full">
      <div className="absolute top-4 left-1/2 flex w-[90%] -translate-x-1/2 gap-2">
      ...
      <span
        className="absolute left-2 top-1/2 grid h-8 w-8 -translate-y-1/2 cursor-pointer place-items-center rounded-full bg-white"
        onClick={handleLeftClick}
      >
        <ChevronLeftIcon className="h-4 w-4 text-secondary" />
      </span>
      <span
        className="absolute right-2 top-1/2 grid h-8 w-8 -translate-y-1/2 cursor-pointer place-items-center rounded-full bg-white"
